...
--CREATE DATABASE SchemaCompareTest_Target;
--GO

USE SchemaCompareTest_Target;
GO
...
-- 정렬변경
CREATE INDEX IDX_TestTable1_Sub1__ColumnB ON TestTable1_Sub1(ColumnB);
GO

-- 스키마는 다르지만 실제로는 같아야 함
CREATE INDEX IDX_TestTable1_Sub1__ColumnAnB ON TestTable1_Sub1(ColumnA ASC, ColumnB);
GO
...
CREATE TABLE TestTable3(
	IDNo INT NOT NULL IDENTITY(1, 1),
	ColumnABC VARCHAR(100) NOT NULL
	CONSTRAINT PK_TestTable3 PRIMARY KEY(
		ColumnABC
	)
);
GO
...
-- 신규 함수
CREATE OR ALTER FUNCTION TestFunction3()
RETURNS VARCHAR(10)
AS
BEGIN
...
AS
BEGIN
	DECLARE @UserNameUse AS VARCHAR(100) = UPPER(@UserName);
	DECLARE @DnT AS VARCHAR(19) = CONVERT(VARCHAR(19), GETDATE(), 121);

	SELECT ('Hello ' + @UserNameUse + ' - ' + @DnT) AS HelloMessage;
END;
GO

-- 신규 프로시저
CREATE OR ALTER PROCEDURE TestProcedure3
AS
BEGIN
	SELECT CONVERT(VARCHAR(10), GETDATE(), 121) AS DnT;
END;
GO
