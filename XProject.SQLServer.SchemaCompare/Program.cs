...
namespace XProject.SQLServer.SchemaCompare
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (isRight == true)
            {
                ...
                using (var workResult = new StreamWriter(reportFilePath, false, Encoding.UTF8))
                {
                    ...
                    try
                    {
                        ...
                        workResult.WriteLine($"> 소스서버(Source server) : {sourceSQL.ServerInfo}");
                        workResult.WriteLine($"> 타겟서버(Target server) : {targetSQL.ServerInfo}");
                        workResult.WriteLine(string.Empty);
                        ...
                        XWork.TableCompare.ExecuteNow(sourceSQL, targetSQL, workResult.WriteLine, schemaDirectory);
                        XWork.ProcedureCompare.ExecuteNow(sourceSQL, targetSQL, workResult.WriteLine, schemaDirectory);
                        XWork.FunctionCompare.ExecuteNow(sourceSQL, targetSQL, workResult.WriteLine, schemaDirectory);
            ...
            else
            {
                ...
                Console.Out.WriteLine("> XProject.SQLServer.SchemaCompare.exe [SourceServer] [TargetServer] {ReportDirectoryPath}");
                Console.Out.WriteLine(string.Empty);
